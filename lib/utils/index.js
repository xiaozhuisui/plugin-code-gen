...
const handleFieldList = (fieldList, form) => {
  // 开始过滤 直接干掉
  const filterList = fieldList.filter(item => !["currentPage", "pageSize", "sorter", "qp-isvId-eq", "qp-isvId-eq", "qp-createUserName-eq", "qp-modifyUserName-eq", "qp-modifyRealName-eq", "sso-sessionid", "x-app-id", "x-isv-id", "x-tenant-id"].includes(item.name));
