...
const handleFieldList = (fieldList, form) => {
  // 开始过滤 直接干掉
  const filterList = fieldList.filter(item => !["currentPage", "pageSize", "sorter", "isvId", "createUserName", "modifyUserName", "modifyRealName", "sso-sessionid", "x-app-id", "x-isv-id", "x-tenant-id", "id"].some(sItem => item.name.includes(sItem)));
